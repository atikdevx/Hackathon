...
| 25 | Paraphrase robustness | PASS | `pytest -m live` with real Gemini `gemini-3.5-flash-lite`: 30/30 (19 paraphrase/percentage/12h-24h/no_op notes, a 3-note call, 10 public samples); local 7B model 29/30 |
...
| 27 | Structured-output / JSON-schema invocation | PASS | Gemini `responseJsonSchema` (`tests/test_gemini_provider.py::test_request_shape_and_text_extraction`); OpenAI `response_format.json_schema` with `strict: true` (`test_openai_reasoning_model_request_shape`); Claude `output_config.format` (`test_request_shape_and_text_extraction`) |
...
| 62 | p95 ≤ 5 s | PASS | real Gemini `gemini-3.5-flash-lite`: 10 public samples median 1.5 s, p95 1.95 s. Via Docker: p95 5.2 s, where 2 requests went to the slower fallback while the free-tier quota benched the primary. A billing-enabled key avoids that. |
| 63 | Stable across repeated requests | PASS | `test_repeated_api_requests_are_stable` (15×); 25+ live requests without errors |
| 64 | Provider timeout, rate limit, outage, malformed output handled | PASS | automatic model fallback on 503/429/5xx/timeout (`test_transient_failure_benches_model_and_next_call_uses_fallback`, `test_interpreter_recovers_within_one_request_via_fallback`; observed live in Docker);  `tests/test_reliability.py`, `tests/test_gemini_provider.py`, `tests/test_openai_provider.py`, `tests/test_anthropic_provider.py` |
